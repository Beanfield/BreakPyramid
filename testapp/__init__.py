...
def main(global_config, **settings):
    ...
    config.add_subscriber("testapp.subscribers.create_url_generator",
        "pyramid.events.ContextFound")
    config.add_subscriber("testapp.subscribers.add_renderer_globals",
                          "pyramid.events.BeforeRender")

    # Set up view handlers
    config.include("testapp.handlers")

    # Set up other routes and views
    # ** If you have non-handler views, create create a ``testapp.views``
    # ** module for them and uncomment the next line.
    #
    #config.scan("testapp.views")

    # Mount a static view overlay onto "/". This will serve, e.g.:
    # ** "/robots.txt" from "testapp/static/robots.txt" and
    # ** "/images/logo.png" from "testapp/static/images/logo.png".
    #
    config.add_static_route("testapp", "static", cache_max_age=3600)
    ...
    # ** "testapp/static" directory. The second serves URLs under 
